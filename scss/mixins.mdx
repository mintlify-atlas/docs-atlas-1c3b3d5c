---
title: "SCSS Mixin Library Reference"
sidebarTitle: "Mixins"
...
Generates `h1`–`h6` selectors with decreasing font sizes. Each heading level is 0.2 em smaller than the previous, starting from `$size`.

```scss
.article {
  @include heading(2em);
  // → h1 { font-size: 2em }  h2 { font-size: 1.8em }  … h6 { font-size: 1em }
}
```
...
Wraps the high-DPI image swap in a `min-device-pixel-ratio: 2` / `-webkit-min-device-pixel-ratio: 2` media query.
...
Emits `-webkit-backface-visibility` and `backface-visibility`, useful when applying 3D transforms to prevent flickering.
...
Emits `-webkit-transform`, `-ms-transform`, and `transform`.
...
Emits `-webkit-transform-style` and `transform-style`. Typically used with `preserve-3d`.
...
Emits `-webkit-transition` and `transition`.
...
Wraps the animation block in both a `@-webkit-keyframes` and `@keyframes` at-rule.
...
Generates `::-webkit-input-placeholder`, `:-moz-placeholder`, `::-moz-placeholder`, `:-ms-input-placeholder`, and `::placeholder` selectors in one go.
...
Dynamically generates a `.bg-{hex}` utility class for the given color, stripping the `#` from the hex value to form a valid class name. Uses `to-string()` and `str-replace()` from `_functions.scss` internally.
